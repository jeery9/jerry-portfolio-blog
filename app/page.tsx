...
import { BookOpen, Video, Quote, User, MessageSquare, Globe, ArrowRight, Play, Heart, Menu, X, Send } from "lucide-react";
...
setBroadcastStatus("Connecting to global cluster...");

try {
  const { error } = await supabase
    .from("community_hub")
    .insert([{ message: inputMessage.trim() }]);

  if (error) {
    setBroadcastStatus(`Broadcast failed: ${error.message}`);
  } else {
    setBroadcastStatus("Message cleanly broadcasted to global hub!");
    setInputMessage("");

    setTimeout(() => setBroadcastStatus(""), 4000);
  }
} catch (err) {
  setBroadcastStatus("Cross-origin transmission failure.");
} finally {
...
<div className="flex flex-col md:flex-row h-screen bg-neutral-950 text-neutral-100 font-sans antialiased selection:bg-emerald-500 selection:text-neutral-950 overflow-hidden">
  ...
  <aside
    ...
    } md:relative md:translate-x-0 transition-transform duration-300 ease-in-out w-64 border-r border-neutral-800 bg-neutral-900/90 md:bg-neutral-900/50 backdrop-blur-md flex flex-col justify-between p-6 z-40 h-full pt-20 md:pt-6`}
  >
    <div>
      ...
      {activeTab === "portfolio" && (
        <div className="space-y-6 animate-in fade-in duration-300">
          <div className="border-b border-neutral-800 pb-6">
            <span className="text-xs uppercase tracking-widest text-emerald-500 font-mono">
              Portfolio
            </span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="p-6 bg-neutral-900/60 rounded-xl border border-neutral-800/80">
              ...
              <p className="text-sm text-neutral-400">
                Deploying cloud-native architecture to host corporate blog feeds, content streaming panels, and interactive chat matrix hubs globally.
              </p>
            </div>
            ...
            <div className="p-6 bg-neutral-900/60 rounded-xl border border-neutral-800/80">
              ...
              <p className="text-sm text-neutral-400">
                Engineered with Next.js frameworks, styled via fluid utility layout arrays, and scaling dynamically with PostgreSQL cloud foundations.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-4">
            {blogPosts.map((post) => (
              <Link
                href={`/blog/${post.id}`}
                key={post.id}
                className="group block p-6 bg-neutral-900/40 hover:bg-neutral-900/80 rounded-xl border border-neutral-800 transition-all"
              >
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-1">
                  <h3 className="text-lg font-semibold text-neutral-200 group-hover:text-emerald-400 transition-colors">
                    {post.title}
                  </h3>
      ...
      {activeTab === "media" && (
        <div className="space-y-6 animate-in fade-in duration-300">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            <div className="relative aspect-[9/16] bg-neutral-900 rounded-xl border border-neutral-800 flex items-center justify-center group overflow-hidden cursor-pointer shadow-lg">
              
              <div className="absolute inset-0 bg-neutral-950/40 group-hover:bg-neutral-950/20 transition-colors" />

              <div className="h-12 w-12 rounded-full bg-neutral-800/80 border border-neutral-700 flex items-center justify-center z-10 group-hover:scale-110 transition-transform">
